...
import { supabase, type Profile, type TryOnResult } from '../lib/supabase';
import { fileToBase64 } from '../lib/geminiApi';
...
export default function ProfileView() {
  ...
  const handleDefaultImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .update({ default_tryon_image: dataUrl })
        .eq('id', user.id)
        .select();
  ...
  const handleRemoveDefaultImage = async () => {
    ...
    try {
      await supabase
        .from('profiles')
        .update({ default_tryon_image: null })
        .eq('id', user.id);
  ...
  const handleDeleteTryOn = async (tryOnId: string) => {
    ...
    try {
      ...
      setTryOnHistory(prev => prev.filter(t => t.id !== tryOnId));
    } catch (err) {
